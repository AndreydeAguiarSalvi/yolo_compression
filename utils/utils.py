...
def compute_loss(p, targets, model, no_detach=False):  # predictions, targets, model
    ...
    if no_detach: return lbox, lobj, lcls
    loss = lbox + lobj + lcls
...
def plot_results(start=0, stop=0, bucket='', id=(), folder =''):  # from utils.utils import *; plot_results()
    ...
    for f in sorted(files):
        ...
        for i in range(10):
            ...
            ax[i].set_title(s[i])
            ax[i].grid(color='gray', linestyle=':', linewidth=1)
