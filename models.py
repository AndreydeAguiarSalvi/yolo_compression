...
class YOLO_CS(nn.Module):
    # YOLOv3 object detection model

    def __init__(self, cfg, img_size=(416, 416), arc='default'):
        super(YOLO_CS, self).__init__()
    ...
    def forward(self, x, masked_weight, var=None):
        img_size = x.shape[-2:]
        yolo_out, out = [], []
        verbose = False
        if verbose:
        ...
        for i, (mdef, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if mtype in ['convolutional']:
                ...
                # bias = module.bias
                x = F.conv2d(x, masked_weight[mask_keys[mask_counter]], stride = stride, padding = padding, groups = groups)
                mask_counter += 1
